...
model_hierarchical <- readRDS("model_hierarchical_5_15_upd.rds") 
```
...
The approach we had was much simpler. Intuitively, we hypothesized that each team in France had some abilities to attack and defend that we would like to evaluate based on the previous results. Then, when looking at a fixture we could combine those values to know how much goals the home and away sides are likely to score or concede. We also added a some fixed bonus to the home side to represent the home advantage in football. Indeed, as they are playing in front of their own fans, it has been shown that home teams tend to win a little bit more frequently and we wanted to take this information into account as well. Knowing this information is particularly important in our case study because it enables us to simulate games were the home advantage's influence is removed (games that are played without public because of the COVID restrictions for instance). From the plot below, we can see from our data how the scores depending on whether team plays home or away are distributed, and it is clear that teams playing home perform better. 

```{r}
require(gridExtra)
plot1 <- ggplot(df_res, aes(x=h.g.))+geom_density(color="darkblue", fill="lightblue")+geom_vline(xintercept = mean(df_res$h.g.), color="blue", linetype="dashed", size=1)
plot2 <- ggplot(df_res, aes(x=a.g.))+geom_density(color="coral", fill="lightcoral")+geom_vline(xintercept = mean(df_res$a.g.), color="blue", linetype="dashed", size=1)
...
For this analysis, we have used score sheets data alongside the composition of the remaining fixtures of the 2019-2020 Ligue 1 season from the internet website Soccerway.com.
Considering the fact that our research question is derived from a recent event at the time of writing, it is unlikely to our knowledge than other existing analysis have been made on the same data set. However, it is worth to mention that our work takes inspiration from other academic works that have been performed on different leagues and seasons, works that are themselves based on decades of scientific research made on sports analytics and prediction models.

Data set contains home team name, away team name, identifier for a home team, identifier for an away team, number of goals scored by home team and number of goals scored by away team. The Table 1 demonstrates basic structure of the data, we also can see that team identifiers are uniquely associated with each team. For example, 'Nice' always associated with identifier 11, whether they play home or away, as in the games 228 and 146. 

```{r, echo=FALSE, results='asis', message=FALSE}
set.seed(42)
Sys.setlocale('LC_ALL','C')
df_merged = merge(df_res, df_teams, by.x='home.team', by.y = 'index')
names(df_merged)[names(df_merged) == 'team'] <- 'home.team.name'
df_merged = merge(df_merged, df_teams, by.x='away.team', by.y = 'index')
names(df_merged)[names(df_merged) == 'team'] <- 'away.team.name'
col_order <- c("home.team.name", "away.team.name", "home.team",
                "away.team", "h.g.", "a.g.")
df_merged <- df_merged[, col_order]
df_merged <- df_merged[sample(nrow(df_merged)),]
knitr::kable(df_merged[1:5, ], caption = "Table 1: The data for the 2019-2020 Ligue 1 season")
```
...
As we have said, our work took inspiration from other academic publications. In particular, "Bayesian hierarchical model for the prediction of football results" by Baio and Blangiardo has been an great source of knowledge and a corner stone for the models we have decided to implement. As its name suggest it, the paper uses a hierarchical model to predict the attacking and defensive strength of the teams.

The hyperpriors $\mu_{att}$,$\sigma_{att}$ and $\mu_{def}$,$\sigma_{def}$  define the mean and standard deviation of a team's capabilities on both the attacking and defensive side.
Leading the following description of the team-specific effects (for each team $t$ in $1,...,T$) modeled as exchangeable from a common normal distribution:
$$
\text{att}_{t} \sim \text{Normal}(\mu_{att}, \sigma_{att}) \\
\text{def}_{t} \sim \text{Normal}(\mu_{def}, \sigma_{def}) \\
$$
The home advantage is modeled by the random variable home.
...
In our own model, we found out that the attacking and defending hyperpriors did not need to be defined as strictly as they are in the paper. In particular, since we only care about the spreading of the team-specific effect $att_t$ and $def_t$ to differentiate the teams (the information about the mean values are lost with the sum-to-zero constraint), we figured out that we might as well eliminate the hyperpriors relative to the mean values and model the team specific effect simply as:
$$
\text{att}_t \sim \text{Normal}(0, \sigma_{att}) \\


\text{def}_t \sim \text{Normal}(0, \sigma_{def})
$$
...
Since it is really difficult to assess objectively the home advantage and the attacking and offensive capabilities of the teams as we do not have a population basis, we decided to stick to the following non informative set of hyperpriors and priors:

$$
\sigma_{att} \sim \text{Gamma}(5, 15) \\
\sigma_{def} \sim \text{Gamma}(5, 15)
$$
...
For the same reasons, the team-specific effects (for each team $t$ in $1,...,T$) are modeled as follows in the separate model:
$$
...
For Separate model we run 4 chains with 12000 iteration, half of which are warm up. 


```{r eval=FALSE}
stan_data <- list(N=nrow(df_res), nteams=length(unique(df_res$home.team)),
                  y1=df_res$h.g., y2=df_res$a.g, hometeam=df_res$home.team,
                  awayteam=df_res$away.team,
                  next_N=nrow(df_next),
                  next_hometeam=df_next$home.team, next_awayteam=df_next$away.team, 
                  sigma_att=1, sigma_def=1)
sm <- rstan::stan_model(file = "football_separate.stan")
model_separate <- rstan::sampling(sm, data = stan_data, chains=4, 
                                  iter=12000, seed=42)
```

For Hierarchical model 4 chains with 2000 iterations, 1000 warm up rounds were sufficient. 
```{r eval=FALSE}
sm <- rstan::stan_model(file = "model_init.stan")
model_hierarchical <- rstan::sampling(sm, data = stan_data, seed=42)
```
...
We can see from the plot of $\hat{R}$ values of separate model that all values are close to 1, thus the model has converged. 
```{r}
rhats_sep <- rhat(model_separate)
mcmc_rhat(rhats_sep)
```
We also conclude that hierarchical model have converged as all of $\hat{R}$ values are around 1, thus the model has converged.
```{r}
...
For hierarchical model there are also no parameters with ratio on $n_{eff}$ to $N$ less than 0.1, we do not have autocorrelation. 
```{r}
...
Running hierarchical model with default parameters was enough for model convergence.
...
For posterior predictive checks we reproduce predictions $y\_rep_{g1}$ and $y\_rep_{g2}$ for each game using proposed models and check results for validity. 
Comparing histogram of $y_{g1}$ to histograms of $y\_rep_{g1}$, we can see that posterior draws for goals scored by the home team are reasonable for separate model, but sometimes separate model predicts values which are too high, for example scores equal to 9-10.
...
```
Comparing histogram of $y_{g2}$ to histograms of $y\_rep_{g2}$, we can see that posterior draws for goals scored by the away team are reasonable for separate model.
```{r}
...
For scores of the home teams let's look at a comparison of the distribution of $y_{g1}$ and the distributions of some of the simulated by separate model datasets in the $y\_rep_{g1}$ matrix. In the plot below, the dark line is the distribution of the observed outcomes $y_{g1}$ and each of the 100 lighter lines is the kernel density estimate of one of the replications of $y_{g1}$ from the posterior predictive distribution (i.e., one of the rows in $y\_rep_{g1}$). This plot makes it easy to see that separate model predicts higher values then observed ones. 

```{r}
ppc_dens_overlay(y_1, y_rep1_sep[1:100, ])
```
For scores of the away teams let's look at a comparison of the distribution of $y_{g2}$ and the distributions of some of the simulated by separate model datasets in the $y\_rep_{g2}$ matrix. Overall distributions look similar, but sometimes separate model predicts more zeros then there are zeros in observed data.
```{r}
...
We can see from plot below, that actual proportion of zeros in observed data of home team scores is lower then the proportion of zeros in predictions generated by separate model. 
```{r}
prop_zero <- function(x) mean(x == 0)
print(prop_zero(y_1))
ppc_stat(y_1, y_rep1_sep, stat="prop_zero")
```
Plot shows that proportion of zeros in observed away team scores mostly lower than predictions of proportion of zeros by separate model.
```{r}
print(prop_zero(y_2))
...
Comparison of mean and variance statistics for observed home team scores and predicted by separate model demonstrates that both means and variances of predicted values tend to be higher then actual values.

```{r}
ppc_stat_2d(y_1, y_rep1_sep, stat=c("mean", "sd"))
```

The same could be concluded for away team scores, means and variances are tend to be higher then actual ones.
```{r}
ppc_stat_2d(y_2, y_rep2_sep, stat=c("mean", "sd"))
```

Distribution of predictive errors for home team and away team scores.
...
Compare histogram of $y_{g1}$ and $y_{g2}$ to histograms of $y\_rep_{g1}$ and $y\_rep_{g2}$, respectively. We can see that hierarchical model does not predict values as high as separate model for home teams.
...
Compare density estimate of $y_{g1}$ and $y_{g2}$ to density estimates of a bunch of $y\_rep_{g1}$ and $y\_rep_{g2}$ respectively. Comparing distributions of observed and predicted values both for the home and away teams, for home teams there are occasions when hierarchical model predict values which are too high, and for the away teams there are predictions with too many zero scores.
...
Compare proportions of zeros in $y_{g1}$ and $y_{g2}$ to the distribution of that proportion over all $y\_rep_{g1}$ and $y\_rep_{g2}$. predicted proportions of zeros are tend to be higher for home teams, but overall separate model performs worse than hierarchical in this case. Predictions of proportion of zeros for away teams are higher than observed ones.
...
For home teams predicted values generated by hierarchical model have means and variances more similar to observed ones than predictions of separate model. The same can be said for scores of away teams.
...
Distribution of predictive errors for home and away teams.
...
## 11. Sensitivity analysis with respect to prior choices


Trying different priors for separate model
...
```
We can conclude that priors $att_t \sim N(0, 1)$, $def_t \sim N(0, 1)$ are better for separate model according to PSIS-LOO diagnostic.

Trying different priors for hierarchical model.
...
```
The difference between proposed priors is not significant according to PSIS-LOO diagnostic. 
...
But obviously, one could think of many other variables that could have an impact on the result and include them in the model to build more robust predictions.

Modelling the number of goal scored by each team has two issues. Firstly, the model cannot differenciate a team that has been dominating the game from another that has been very fortunate if the scoreline is the same. This is a problem as a scoreline does not necesserely give all the information about the strengh of a team, especially in a sport like football were big teams frequently lose against mildly succesful sides. Considering an "advanced" metric like expected goals (xG) instead of actual goals could make some improvements in that regard. Seccondly, we have faced an issue when it comes to decide if a game would end up in a draw. If we take the mean number of goal scored by each from 1000 or 2000 sampled values, there are very little chance that the 2 values would be exactly the same, meaning that the match would not be predicted as a draw. Hence, it becomes very difficult to predict the actual game table with this method. Using a threshold allowed us to end up with an actual league table but the approach can be questionable, one could have use each sequence of draws individually to predict a lot of plausible league tables and then look at the frequencies for each team to end up at each position.
...
Our 2019-20 Ligue 1 prediction models were able to predict some significant changes in the league table if the French teams were to play the 10 remaining games of that season. However, those predicted changes do not occur in the relegation spots as Toulouse and Amiens are still predicted to finish last and second-last these season, nor in the Champions League group stages qualifying spots as Paris and Marseille are predicted to end up in the first and second place. For the 3rd place race however, there could be arguments for Rennes or Lille and both team would have undoubtedly offer us a breathtaking finish with a lot of suspence.

The topic chosen for our project is initially quite interesting, being able to predict football match results based on a team's previous performance is challenging. During the project realization we performed thorough analysis on previous researches of the case, we learned how to design a mathematical model to solve the problem, what is identifiability constraint and what measures could be done to impose these constraints, why hierarchical model is suitable for this kind of task, and what could be done to enhance the performance of the hierarchical model. Additionally we got solid understanding of log-linear random effect models and how the parameters learned during the simulation can be interpreted. 

You can find the GitHub project repository through the following link: https://github.com/4nar/bda_project



# 14. Self-reflection on what the group learned while making the project.


Along with hierarchical model suggested by previous researches we designed separate model to compare it to the results of hierarchical model. And as results suggest even if hierarchical model performs better, separate model also has it's advantages and performs quite well. Also, techniques to tune the parameters for Stan model, to compare model results as PSIS-LOO, to chose appropriate priors, to perform predictive posteriors checks were used and some deeper understandings of these techniques were gained. 

During the project, we have discovered how useful and powerful bayesian inference and MCMC methods can be to design very accurate models. At first glance, predicting the outcome football games looked like an impossible tasks, but in the end, simple models like the one we designed in 3 weeks were already able to make sensible predictions. The possibilities to apply the same precepts in industrial context are incredibely wide and we think that understanding and somewhat mastering these methods will be essential for any good Data Scientist in the future.
